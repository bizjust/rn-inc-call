...
//    player.stop();
  ...
  /* private Notification buildNotification(Context context, Intent intent) {
    ...
    if(bundle.getString("notificationColor")!=null){
      ...
      // notificationBuilder.setColor(ContextCompat.getColor(context, R.color.notification_action_accept));
    }
  ...
  private Notification buildNotification(Context context, Intent intent) {
    ...
    NotificationCompat.Builder notificationBuilder;
    RemoteViews notificationLayout = new RemoteViews(getPackageName(), R.layout.activity_call_notify);
//    Button btn = (Button) findViewById(R.id.btnAccept);
    notificationLayout.setOnClickPendingIntent(R.id.btnAccept, onButtonNotificationClick(1,Constants.ACTION_PRESS_ANSWER_CALL));
    notificationLayout.setOnClickPendingIntent(R.id.btnDecline, onButtonNotificationClick(0,Constants.ACTION_PRESS_DECLINE_CALL));
    notificationLayout.setTextViewText(R.id.txtUser, bundle.getString("name"));
